...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['param'])) {
    PartySetEquip($_GET['param']);
}

function PartySetEquip($param) {
    ...
    $file['party']['avatar']['armament_uniqs'] = $arms_array;
        ...
        $home_file = "../../saves/players/0/d2mode/home.json";

        $home = json_decode(file_get_contents($home_file), true);

        $home['avatar']['armament_uniqs'] = $arms_array;

        file_put_contents($home_file, json_encode($home));
